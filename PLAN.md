...
### Step 15 — Group / plural `align_to` on `set_*_targets` (next up)

The single-segment `align_to` landed in Step 10.5. This step extends it to the *plural* `Clock.set_*_targets` (and their `utilities` forwarders + `__init__` re-exports), so a whole run of segments can be bent collectively to land on a phase/coordinate.

**API — `Clock.set_*_targets(targets, whens, curve_shapes=None, truncate=True, align_to=None)`** (add the `align_to` param). `align_to` accepts:
- **A single `ResolvableMoment`** → align the *whole* run at its end. Sugar for `[None]*(N-1) + [value]`.
- **A per-segment list** (same length as `targets`), entries `None` or a `ResolvableMoment`. A non-`None` entry at index *i* bends every segment since the previous alignment point (or the start of the call) up through *i* so the run collectively lands on that target. So `[None, None, phaseX, None, phaseY]` defines two aligned runs: segments 0–2 (landing on `phaseX`) and 3–4 (landing on `phaseY`); a trailing run with no alignment is just built normally.

**Constraints / semantics:**
- A group-aligned run (length > 1) **must be single-axis** — every `when` in that run pins the same axis (all beats or all time) — because the distribution policy holds one axis fixed and pushes the other. Mixed axes within an aligned run → `ValueError`. (A *single value* over a mixed-axis `whens` list therefore also raises, since it forms one run over the whole list.)
- Each `align_to` entry must be on the run's **free** axis (opposite the run's pinned axis), same rule as the singular case; a `MetricPhaseTarget` with `units=None` infers it.
- The "explicit `curve_shape` discarded with a warning for a *fixed* align target" rule is **run-length-1 only** (a one-segment run is just the singular solve). For a genuine group (length > 1) there is no unique curvature, so `curve_shapes` legitimately seed the distribution.
- `whens` still resolve against *now* and must be strictly increasing in clock-time (unchanged from Step 10.5).

**Implementation — drive grouping from `_apply_targets`** (`clock.py`). Walk left-to-right as now, but accumulate the indices of the current run; when an `align_to` entry is non-`None` (or at the final segment for a single whole-run value), close the run and solve it:
- **Run length 1** → reuse the existing singular path: pass `alignment_target` straight through to the singular `TempoHistory` setter (`_solve_segment_end_{time,beat}`). No new machinery.
- **Run length > 1** → the underdetermined group solve (N curvature unknowns, 1 endpoint constraint). The distribution policy already exists in `TempoHistory` and is the thing to reuse:
  - **Free axis = TIME** (run pins beats): hold the run's beats, push its total *time* onto the goal via **`_adjust_segments_time_duration(which_segments, goal_total_time)`** — it weights each segment's adjustment by its `get_integral_range()` wiggle room, range-checks feasibility, and the caller restores `self.segments` on `False`. For a phase target, resolve the goal time from the run's current end time via `get_nearest_matching_times` (mirroring `adjust_metric_phase_at_beat` → `adjust_time_at_beat`).
  - **Free axis = BEATS** (run pins time): the proportional beat-stretch-then-time-resolve — stretch the run's beats proportionally so the end beat lands on the target, then re-solve each segment's time. This is what `adjust_beat_at_time` / `adjust_metric_phase_at_time` do.
- **Atomicity:** wrap the whole plural build so a failure anywhere restores the pre-call `self.segments` (the singular setter already snapshots per-segment; the group path needs a call-level snapshot too).

**Reuse vs. rebuild — existing `TempoHistory` group helpers:** `adjust_metric_phase_at_beat` / `adjust_time_at_beat` / `_adjust_segments_time_duration` (TIME-free) and `adjust_metric_phase_at_time` / `adjust_beat_at_time` (BEATS-free) are already present, ported from original clockblocks. **Caveat: the BEATS-free pair references `self.get_beat_wait_from_time_wait(...)`, which does not exist in cb2's `TempoHistory`** (it shows up as an IDE "unresolved attribute" warning) — they are currently *dead and unverified*. Before relying on them, either restore an equivalent time-wait→beat-wait inversion (cb2 has `beat_at_time`/`time_at_beat` to build it from) or reimplement the BEATS-free solve directly. The TIME-free path (`_adjust_segments_time_duration`) is exercised indirectly and trustworthy.

**Cleanup once group is driven from `_apply_targets`:** delete the dead monolithic `TempoHistory.set_*_targets` plural block (still in place only because a `MetricPhaseTarget` passed positionally as `alignment_target` reproduced its old behavior) and any now-orphaned helpers (`adjust_metric_phase_at_beat`/`adjust_metric_phase_at_time` if the new path supersedes them, plus the `MetricPhaseTarget.interpret` tuple-coercion path if nothing else uses it).

**Tests** (`test_module_api.py`): single-value whole-run align lands a multi-segment run on a phase (verify the run's end beat is on-phase and per-segment beats unchanged for the TIME-free case); per-segment list with `None`s producing two independent aligned runs; single value over a mixed-axis `whens` raises; a mixed-axis run raises; unreachable group align rolls the whole curve back. Add the `align_to` param to the three plural `utilities` forwarders and cover the off-thread `NoActiveClockError` path.

See `.claudeConvos/2026-06-10-cb2-tempo-target-moment-api.md` for the design discussion (3-knobs model, single-vs-group distinction).
