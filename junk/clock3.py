...
import threading
import time
from junk.scheduler import Scheduler, get_scheduler
from cb2.tempo_envelope import TempoHistory, TempoEnvelope
from utilities import wait


# def recalculate_child_scheduling(fn):
#     """
#     Decorator applied to methods of Clock that alter tempo. Any children of this clock will need to recalculate and
#     reschedule their wake-up event, since their parent's tempo environment has changed
#     """
#     @wraps(fn)
...
class Clock:
    ...
    def wait(self, dt, units="beats"):
        if units == "beats":
            ...
            # wake_up_beat = self.beat() + self._tempo_history.get_beat_wait_from_time_wait(dt)
